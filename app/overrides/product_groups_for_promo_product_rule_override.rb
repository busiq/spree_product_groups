...
<p class="field products_rule_products_source_field">
  <label><%= radio_button_tag "#{param_prefix}[preferred_group]", false, promotion_rule.preferred_group == false %> <%= t "product_rule.product_source.manual" %></label> &nbsp;
  <label><%= radio_button_tag "#{param_prefix}[preferred_group]", true, promotion_rule.preferred_group == true %> <%= t('product_rule.product_source.group') %></label>
</p>
